...
class UDriveServer {
  ...
  bool allowNetworkAccess = false; // New setting for network access
  String? serverUrl;
  String? networkUrl;
  ...
  Future<bool> startServer(
      {String? customStoragePath, bool enableNetworkAccess = false}) async {
    try {
      if (isRunning) {
      ...
      // Set network access preference
      allowNetworkAccess = enableNetworkAccess;
      ...
      // Start HTTP server - bind to all interfaces if network access is enabled
      ...
      if (allowNetworkAccess) {
        ...
        final networkIP = await _getLocalNetworkIP();
        if (networkIP != null) {
          networkUrl = 'http://$networkIP:$port';
          print('🌐 Network URL: $networkUrl');
          print('� Share this URL with devices on your local network');
        } else {
  ...
  Future<void> _handleStreamRequest(HttpRequest request) async {
    try {
      final filePath =
          Uri.decodeComponent(request.uri.path.substring('/stream/'.length));
      print('🎥 Stream request for: $filePath');
  ...
  Future<String> _loadHtmlFile() async {
    final possiblePaths = [
      ...
      path.join('D:', 'PersonalProjects', 'PrimeWinTools', 'lib', 'udrive',
          'web_assets', 'index.html'),
      path.join('lib', 'udrive', 'web_assets', 'udrive.html'),
...
<body>
  ...
  Future<Map<String, dynamic>> _getNetworkInfo() async {
    ...
    String? primaryUrl = 'http://localhost:$port';
      ...
      try {
        ...
        for (final interface in networkInterfaces) {
          for (final address in interface.addresses) {
            ...
            // Look for private network addresses
            if (ip.startsWith('192.168.') ||
              ...
              // Use the first found network IP as primary
              if (primaryUrl == 'http://localhost:$port') {
                primaryUrl = url;
                autoDetected = true;
              }
            }
    ...
    return {
      ...
      'networkUrl': allowNetworkAccess ? networkUrl : null,
      'localUrl': 'http://localhost:$port',
      'autoDetected': autoDetected,
      'networkAccessEnabled': allowNetworkAccess,
    };
  }
  ...
  Future<String?> _getLocalNetworkIP() async {
    try {
      ...
      for (final interface in interfaces) {
        for (final address in interface.addresses) {
          final ip = address.address;
          // Look for private network addresses (192.168.x.x, 10.x.x.x, 172.16-31.x.x)
          if (ip.startsWith('192.168.') ||
              ip.startsWith('10.') ||
              (ip.startsWith('172.') && _isPrivateClass172(ip))) {
            return ip;
  ...
  /// Check if IP is in the 172.16.0.0 - 172.31.255.255 range
  ...
  void setNetworkAccess(bool enabled) {
    allowNetworkAccess = enabled;
    print(allowNetworkAccess
        ? '🌐 Network access enabled - server accessible from local network'
        : '💻 Network access disabled - local access only');
  }
}
